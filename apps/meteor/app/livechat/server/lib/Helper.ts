...
import { Livechat, Livechat as LivechatTyped } from './LivechatTyped';
...
export const forwardRoomToDepartment = async (room: IOmnichannelRoom, guest: ILivechatVisitor, transferData: TransferData) => {
	...
	const department = await LivechatDepartment.findOneById<{ allowReceiveForwardOffline: boolean }>(departmentId, {
		projection: {
	...
	if (
		...
		(department?.allowReceiveForwardOffline && !(await Livechat.checkOnlineAgents(departmentId)))
	) {
